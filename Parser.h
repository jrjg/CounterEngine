...
public:
	...
	Type* mpType;
	Variable* mpVariable;
	Object(MemManaged* pInst, Type* pType, Variable* pVariable) : mpInst(pInst), mpType(pType), mpVariable(pVariable) {};
	~Object() {};
}; 
...
private:
	...
	HRESULT convertFromStringToFloat(void* pObject,String<char>* pString);
	HRESULT convertFromStringToBool(void* pObject, String<char>* pString);
	HRESULT convertFromStringToString(void* pObject, String<char>* pString);
	HRESULT convertFromStringToUINT(void* pObject, String<char>* pString);
	HRESULT convertFromStringToLPCWSTR(void* pObject, String<char>* pString);
...
public:
	Parser();
	~Parser() { delete mpVariables; delete mpObjects; delete mpTypes; delete mpOperators; };
	...
	HRESULT declareVariable(char* pTypeName, char* pVariableName, ObjectHandlerFunction pObjectHandlerFunction) { mpVariables->pushBack(new Variable(new String<char>(pVariableName, strlen(pVariableName)), new String<char>(pTypeName, strlen(pTypeName)), pObjectHandlerFunction)); return S_OK; };
	HRESULT registerOperator(char* pName, OperatorCode code) { mpOperators->pushBack(new Operator(new String<char>(pName,strlen(pName)) ,code)); return S_OK; };
